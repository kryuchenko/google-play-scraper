...
func (c *Client) SitemapIndexURLs(ctx context.Context) ([]string, error) {
	ctx, endTask := startTask(ctx, traceTaskSitemapIndexURLs)
	defer endTask()

	return c.sitemapIndexURLs(ctx)
}

// sitemapIndexURLs is SitemapIndexURLs without the trace task, for the callers
// inside this package that already opened one of their own.
func (c *Client) sitemapIndexURLs(ctx context.Context) ([]string, error) {
...
func (c *Client) SitemapShards(ctx context.Context, indexURL string) ([]string, error) {
	ctx, endTask := startTask(ctx, traceTaskSitemapShards)
	defer endTask()

	return c.sitemapShards(ctx, indexURL)
}

// sitemapShards is SitemapShards without the trace task. AllSitemapShards calls
// it once per advertised index, which is where opening a task per call would
// become a task per loop iteration.
func (c *Client) sitemapShards(ctx context.Context, indexURL string) ([]string, error) {
...
//
// The scan is not an XML parser and does not check what encloses a <loc>: a
// <loc> outside its <sitemap> (or <url>) wrapper is read, where a document tree
// would ignore it. Every index and shard Google serves wraps every <loc>, so
// the two agree on real input; the differential fuzz test excludes the
// unwrapped case from its property rather than paying for element tracking in a
// scan that exists to avoid building a tree at all.
...
func (c *Client) AllSitemapShards(ctx context.Context) ([]string, error) {
	ctx, endTask := startTask(ctx, traceTaskAllSitemapShards)
	defer endTask()
...
func (c *Client) SitemapShardPackages(ctx context.Context, shardURL string) ([]string, error) {
	ctx, endTask := startTask(ctx, traceTaskSitemapShardPackages)
	defer endTask()

	return c.sitemapShardPackages(ctx, shardURL)
}

// sitemapShardPackages is SitemapShardPackages without the trace task. A sweep
// calls it once per shard, and 83k tasks would describe the sweep no better
// than one does while costing a derived context each.
func (c *Client) sitemapShardPackages(ctx context.Context, shardURL string) ([]string, error) {
...
//
// Like indexShards, the scan does not check what encloses a <loc>: one outside
// its <url> wrapper is read, where a document tree would ignore it. It still
// has to carry /store/apps/details and survive appPackageFromLoc to be
// admitted, and no shard Google serves has one, so the differential fuzz test
// excludes the unwrapped case from its property rather than teaching a
// streaming scan to track elements across chunk boundaries.
