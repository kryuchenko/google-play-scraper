...
// classifyAvailability interprets the multiplexed availability node -- Ws7gDc
// field 19, at [18][0] of a dense app record -- into a region-level Status:
//
...
//     this region; the node is an empty array)
//
// It never returns StatusNotFound or StatusError: those are HTTP/transport
// outcomes decided before the body is parsed.
//
// The node is read by field number rather than by index because the callers no
// longer agree on the encoding. The page parser passes the dense record, where
// field 19 sits at index 18; an availability probe asks for that field alone,
// and a one-field answer is positional here but a map keyed by field number for
// other fields (see ws7gdcFieldAvailability). digestField reads both. A bare
// getPath(appData, 18, 0) reads index 18 of a map as the key "18" -- field 18,
// not this one -- silently and with a plausible-looking value.
func classifyAvailability(appData any) Status {
	if toInt(getPath(digestField(appData, ws7gdcFieldAvailability), 0)) == 2 {
