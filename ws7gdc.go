...
// answers in. A request that fails marks every app in that chunk and leaves the
...
func (c *Client) AppsMany(ctx context.Context, appIDs []string, opts AppOptions) []AppResult {
	...
	for _, chunk := range chunked(appIDs, maxRPCsPerRequest) {
		...
		for i, id := range chunk {
			...
			calls = append(calls, appDetailsRPC(id))
		...
		for j, i := range slots {
			...
			id := chunk[i]
			r.App, r.Err = parseAppRPCPayload(payloads[j], id, appPageURL(id, opts.Lang, opts.Country))
		}
		base += len(chunk)
	}
	return out
