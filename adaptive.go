...
func WithAdaptiveThrottle(p AdaptivePolicy) ClientOption {
	...
	// Max < Min is already normalised above, so this is well defined. A Start
	// outside the bounds is the same promise broken as an out-of-bounds
	// WithThrottle: the interval has to be inside the policy before the first
	// request, not after the first observation moves it there.
	if p.Start > 0 {
		p.Start = min(max(p.Start, p.Min), p.Max)
	}
...
func (c *Client) observe(ctx context.Context, status int, rtt time.Duration, err error) {
	if c.adaptive == nil {
		return
	}
	c.throttleLock.Lock()
	before := c.throttle
	reason := c.adjust(status, rtt, err)
	after := c.throttle
	c.throttleLock.Unlock()
	c.logThrottle(ctx, before, after, reason)
}

// logThrottle records a change of interval. It runs outside throttleLock: a
// handler may be writing to a file, and that lock is the one every request
// takes to reserve its slot.
//
// The context is the observing request's, not Background: a move of the
// interval is caused by one response, and a handler that cannot see which one
// has a number with nothing to attribute it to.
func (c *Client) logThrottle(ctx context.Context, before, after time.Duration, reason string) {
	if after == before {
		return
	}
	c.log().LogAttrs(ctx, slog.LevelDebug, "throttle",
		slog.Duration("from", before), slog.Duration("to", after), slog.String("reason", reason))
}

// adjust applies one observation to the interval and names the signal behind
// the decision. Callers hold throttleLock; whether the interval actually moved
// is for them to compare, since a decrease at the ceiling changes nothing.
func (c *Client) adjust(status int, rtt time.Duration, err error) string {
	p := c.adaptive
...
func (c *Client) retryAfterHint(ctx context.Context, err error) {
	if c.adaptive == nil {
		return
	}
	var se *StatusError
	...
	after := c.throttle
	c.throttleLock.Unlock()
	c.logThrottle(ctx, before, after, "retry-after")
}
