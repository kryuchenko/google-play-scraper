...
func TestAdaptiveSpeedsUpAfterACleanWindow(t *testing.T) {
	...
	for range 4 {
		c.observe(context.Background(), http.StatusOK, time.Millisecond, nil)
	}
	if got := c.CurrentThrottle(); got != time.Second {
...
func TestAdaptiveBacksOffHardOnRejection(t *testing.T) {
	...
	for _, status := range []int{http.StatusTooManyRequests, http.StatusServiceUnavailable} {
		...
		c.observe(context.Background(), status, time.Millisecond, nil)
		if got := c.CurrentThrottle(); got != time.Second {
...
func TestAdaptiveNeedsAStreakBeforeActingOnLatency(t *testing.T) {
	...
	for range 20 {
		c.observe(context.Background(), http.StatusOK, 100*time.Millisecond, nil)
	}
	before := c.CurrentThrottle()

	// Four slow responses in a row: suggestive, not yet evidence.
	for range 4 {
		c.observe(context.Background(), http.StatusOK, 300*time.Millisecond, nil)
	}
...
func TestAdaptiveGradesTheSignal(t *testing.T) {
	...
	hard := newClient()
	hard.observe(context.Background(), http.StatusTooManyRequests, time.Millisecond, nil)
	if got := hard.CurrentThrottle(); got != 400*time.Millisecond {
		t.Errorf("429: throttle = %v, want the full 4x decrease", got)
	}

	soft := newClient()
	soft.observe(context.Background(), 500, time.Millisecond, nil)
	...
	unavailable.observe(context.Background(), http.StatusServiceUnavailable, time.Millisecond, nil)
...
func TestAdaptiveHonoursRetryAfter(t *testing.T) {
	...
	c.retryAfterHint(context.Background(), &StatusError{Code: 429, RetryAfter: 5 * time.Second})
	...
	c.retryAfterHint(context.Background(), &StatusError{Code: 429, RetryAfter: time.Hour})
...
func TestAdaptiveIsOptIn(t *testing.T) {
	...
	c.observe(context.Background(), http.StatusTooManyRequests, time.Second, nil)
	c.observe(context.Background(), http.StatusOK, time.Microsecond, nil)
...
// "Max is never exceeded" is a promise about the client, not about the order
// its options were written in. WithThrottle applied after WithAdaptiveThrottle
// used to set an interval the policy would never have chosen, and an
// out-of-range Start was never clamped at all -- so a run could begin a minute
// apart under a policy whose ceiling is two seconds, and only come inside the
// bounds after the first observation moved it there.
//
// Built through NewClient rather than adaptiveClient: the clamp lives at the
// end of the constructor, which is the only place that can see every option.
func TestAdaptiveBoundsHoldAtConstruction(t *testing.T) {
	const (
		lo = 100 * time.Millisecond
		hi = 200 * time.Millisecond
	)
	policy := AdaptivePolicy{Min: lo, Max: hi}
	...
	for _, tc := range []struct {
		...
		{"throttle above Max, adaptive first",
			[]ClientOption{WithAdaptiveThrottle(policy), WithThrottle(5 * time.Second)}, hi},
		{"throttle above Max, adaptive last",
			[]ClientOption{WithThrottle(5 * time.Second), WithAdaptiveThrottle(policy)}, hi},
		{"throttle below Min, adaptive first",
			[]ClientOption{WithAdaptiveThrottle(policy), WithThrottle(time.Millisecond)}, lo},
		{"throttle below Min, adaptive last",
			[]ClientOption{WithThrottle(time.Millisecond), WithAdaptiveThrottle(policy)}, lo},
		{"Start above Max",
			[]ClientOption{WithAdaptiveThrottle(AdaptivePolicy{Min: lo, Max: hi, Start: time.Minute})}, hi},
		{"Start below Min",
			[]ClientOption{WithAdaptiveThrottle(AdaptivePolicy{Min: lo, Max: hi, Start: time.Millisecond})}, lo},
		{"adaptive alone still begins at Max",
			[]ClientOption{WithAdaptiveThrottle(policy)}, hi},
		{"a fixed throttle with no policy is untouched",
			[]ClientOption{WithThrottle(5 * time.Second)}, 5 * time.Second},
	} {
		t.Run(tc.name, func(t *testing.T) {
			c := NewClient(tc.opts...)
			if got := c.CurrentThrottle(); got != tc.want {
				t.Errorf("client starts at %v, want %v", got, tc.want)
			}
			if c.adaptive != nil && c.adaptive.Start > 0 {
				if c.adaptive.Start < c.adaptive.Min || c.adaptive.Start > c.adaptive.Max {
					t.Errorf("policy kept Start = %v outside [%v, %v]",
						c.adaptive.Start, c.adaptive.Min, c.adaptive.Max)
				}
			}
		})
	}
