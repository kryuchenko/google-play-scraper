...
go test -tags canary -run TestCanary -v -timeout 15m ./...

# just the CLI end-to-end, against production
go test -tags canary -run TestCanaryCLI -v -timeout 15m ./cmd/gpscrape
```
...
- `testdata/` holds a `go.mod` that is not a real module at all. Its only job
  is exclusion. The recorded pages there are 16.0MB against 0.8MB of actual
  library, so without it every consumer of a zero-dependency library downloads
  sixteen megabytes of HTML they will never open. Measured from `git archive
