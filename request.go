...
// operations that fan out: Availability's per-country sweep, the catalog shard
// sweep, DigestsSeq (which takes its own override), and the packed requests of
// AppsMany, PermissionsMany and SuggestMany. FullDetail enrichment does not use
// it: that path packs its lookups into a batched RPC and is two requests, not
// one per result.
//
...
func NewClient(opts ...ClientOption) *Client {
	...
	// Bounds are the caller's and Max is never exceeded. That is a promise
	// about the client, not about option order, so it is enforced once here,
	// after every option has run: WithThrottle may follow WithAdaptiveThrottle
	// and set an interval the policy would never have chosen.
	if c.adaptive != nil {
		c.throttle = min(max(c.throttle, c.adaptive.Min), c.adaptive.Max)
	}

	// Apply the timeout last so it wins regardless of option ordering and
	// applies to a client supplied via WithHTTPClient too. Copied first: the
	// client may be the caller's, shared with the rest of their program, and
	// setting Timeout on it would change every other request they make.
	if c.timeout > 0 {
		cp := *c.httpClient
...
// reservations rather than on the sleep itself. It returns how long it
// held the caller, and ctx.Err() if the context is cancelled before the
// reserved slot arrives.
//
...
func (c *Client) waitThrottle(ctx context.Context) (time.Duration, error) {
	c.throttleLock.Lock()
