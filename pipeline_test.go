...
// hugeErrorBody is a response body of a stated size that records how much of it
// was actually read. The bytes themselves are irrelevant; only the count is.
type hugeErrorBody struct {
	left int
	read *int
}

func (b *hugeErrorBody) Read(p []byte) (int, error) {
	if b.left == 0 {
		return 0, io.EOF
	}
	n := min(len(p), b.left)
...
func (b *hugeErrorBody) Close() error { return nil }

// errorBodyTransport answers everything with one non-200 response whose body is
// as large as it likes. The routing mock cannot express this: its bodies are
// []byte, and the question here is how much of a body the client chooses to
// pull, not what it contains.
...
func (t *errorBodyTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	return &http.Response{
		...
		Body:       &hugeErrorBody{left: t.size, read: &t.read},
		Header:     http.Header{"Retry-After": []string{t.retryAfter}},
		Request:    req,
	}, nil
}

// A non-200 response used to be returned without touching its body, which makes
// net/http drop the connection instead of pooling it -- a fresh handshake per
// retry, at the moment the store is already asking for less traffic. Draining
// fixes that, and the cap is what keeps the fix from becoming its own problem:
// an error page has no size anyone here controls.
func TestNonOKBodyIsDrainedButNotWithoutLimit(t *testing.T) {
	const bodySize = 1 << 20 // 1MB of rate-limit page

	tr := &errorBodyTransport{size: bodySize, retryAfter: "7"}
	c := NewClient(WithHTTPClient(&http.Client{Transport: tr}))
	// One attempt, so the byte count below is one response's worth.
	WithRetry(RetryPolicy{MaxAttempts: 1})(c)

	_, err := c.get(context.Background(), BaseURL+"/robots.txt")

	var se *StatusError
	if !errors.As(err, &se) {
		t.Fatalf("get() = %v, want a *StatusError", err)
	}
	if se.Code != http.StatusTooManyRequests {
		t.Errorf("StatusError.Code = %d, want 429", se.Code)
	}
	// Draining must not swallow what the error is for: Retry-After is the whole
	// reason a 429 is worth more than a generic failure.
	if se.RetryAfter != 7*time.Second {
		t.Errorf("StatusError.RetryAfter = %v, want 7s", se.RetryAfter)
	}

	if tr.read == 0 {
		t.Error("nothing was read from the body; the connection cannot be reused")
	}
	// The margin covers the transport's own buffering, which reads in chunks
	// and can overshoot the limit by one of them.
	if limit := maxErrorBodyDrain + 64<<10; tr.read > limit {
		t.Errorf("read %d bytes of a %d-byte error page, want at most %d",
			tr.read, bodySize, limit)
	}
}
