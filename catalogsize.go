...
// String renders the estimate the way a run reports it: the count, the
// generation it was taken against, and either "exact" or the interval and how
// much of the catalog was read.
...
func (c *Client) CatalogSize(ctx context.Context, opts SizeOptions) (CatalogSize, error) {
	ctx, endTask := startTask(ctx, traceTaskCatalogSize)
	defer endTask()
...
func summarise(gen Generation, counts []float64, total int) CatalogSize {
	...
	// N == 1 is a catalog of one shard: the "sample" is the census, the
	// correction is exactly zero, and the textbook form divides by N-1 to say
	// so -- a 0/0 that came back as a public struct full of NaN.
	fpc := 0.0
	if N > 1 {
