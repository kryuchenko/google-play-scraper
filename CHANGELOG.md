...
- `cmd/gpscrape`, a command-line interface. Newline-delimited JSON on stdout,
  ...
  what was already written stays valid.
      ...
      gpscrape catalog -shards 0-99 > ids.txt

  `gpscrape sync` maintains a catalog snapshot directory as a batch job rather
  than a call. Shard filenames carry the id of the generation that produced
  them, so noticing that nothing has changed costs three requests instead of
  83k — and Google does not regenerate daily. Within a generation shards are
  immutable, which is what makes a sweep resumable: an interrupted run
  continues from the shards it has not reached, and a 404 mid-sweep means the
  generation rolled and the run restarts.
...
- Shard decompression allocates once from the size gzip already states.
  `io.ReadAll` grows from nothing, doubling and copying: an 8MB shard cost
  16.7MB and 6.4ms that way, against 8.07MB and 4.4ms when the buffer starts
  the right size. RFC 1952 puts the uncompressed length in the trailer's
  ISIZE — verified exact on a real shard — and it is clamped before use,
  because those four bytes are chosen by whoever served the response.

  A catalog sweep decompresses 83,445 shards, so this is 167 seconds of CPU
  and some 720GB of garbage it no longer makes.
...
- `gpscrape catalog` became a group of verbs, since keeping a local copy of the
  ...
  magnitude in cost:
      ...
      gpscrape catalog genres    ~3.3k requests   genre changes and removals
  ...
  within a day for a few minutes' work; it cannot find an app never heard of.
  `new` finds those, but reads ranked lists, so an app with no traction never
  appears. Only `sweep` is complete. `diff` prints how much of what the sweep
  found the cheap signal had already reported — the recall figure that decides
  how often the expensive pass has to run, printed unasked because a metric
  behind a flag is a metric nobody has.

  `sync` is now an alias for `catalog sweep`.
...
- `-sample 100` wrote a manifest saying the catalog held no apps: the sampler
  ...
  sweep took as its work. 100 now means the whole catalog, anything outside
  (0,100] is refused, and a sweep that collected nothing no longer publishes a
  manifest at all.
...
- `gpscrape sync -check` created the snapshot directory as a side effect of
  asking a question, and printed a sentence to stdout while its sibling branch
  printed to stderr. It now writes nothing and emits one JSON record, like
  every other command.
...
### Changed (on-disk formats)

- `genres.tsv` changed meaning. It was "what the last run resolved" and is now
  "everything ever resolved, minus what was seen to disappear". The merge is
  what stops a transient error deleting an app, but it means the table has no
  removal path for an id that falls out of the sitemap without ever being
  observed as gone: such a row lives on. The error is zero on the day of
  release and grows monotonically; the rate is not measured. Files written by
  1.3.1 and 1.4.0 are byte-for-byte the same format and different meanings.
  To spot drift, compare `wc -l genres.tsv` against `ids` in the manifest.
...
### Internal (measurement and tooling)
