...
const docTemplate = `{
    ...
    "paths": {
        ...
        "/_/PlayStoreUi/data/batchexecute(qnKhOb)": {
            "post": {
                "description": "POST /_/PlayStoreUi/data/batchexecute?rpcids=qnKhOb. Advances a\nlist, cluster or search result set using a continuation token.\nReturns the batchexecute envelope; []SearchResult plus the next\ntoken are decoded from the inner payload.\nEmpirical note: Google currently REJECTS this continuation token\nfor list/cluster/search (returns a 200 with a NULL payload on the\nfirst call), so deeper pages are unreachable and callers get only\nthe initial page. Review pagination uses a different RPC (oCPfdb)\nand still works.",
