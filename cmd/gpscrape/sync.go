...
// deltaName is the one place the delta's filename is spelled. It was spelled
// in two: the writer produced delta-<from>-to-<to>.json and the pruner looked
// for delta-<gen>.json, a name that has never existed -- so -keep deleted
// snapshots and manifests and left every delta behind for good.
func deltaName(from, to googleplayscraper.Generation) string {
	return fmt.Sprintf("delta-%s-to-%s.json", from.ID(), to.ID())
}
...
func cmdSync(args []string) error {
	// -check answered a question `catalog check` answers better: in two
	// requests instead of three -- it listed every shard to learn what
	// SitemapGeneration already says -- and with the generation's build time,
	// age and shard count beside it. Two surfaces over one implementation is
	// what removed `sync`. Refused by name rather than by flag's "flag
	// provided but not defined", which would say nothing about where to go.
	for _, a := range args {
		if a == "-check" || a == "--check" {
			return fmt.Errorf("catalog sweep no longer takes -check; `gpscrape catalog check` " +
				"reports the same thing in 2 requests instead of 3, with built, ageHours and shards")
		}
	}

	c := newCommon("catalog sweep")
	...
	keep := c.fs.Int("keep", 0, "after a successful sweep, keep only this many of the newest "+
		"snapshots and delete older ones (0 = keep every generation)")
	...
	if *sample != 0 && (*sample < 0 || *sample > 100 || math.IsNaN(*sample)) {
		return fmt.Errorf("-sample %v: a share of the catalog is above 0 and at most 100, "+
			"or 0 for the whole catalog", *sample)
	}
	// 0 is the flag's own default and has always meant "all", so it cannot be
	// rejected -- but typed on purpose it reads as "a tiny sample", and the
	// reader who needs to be told otherwise is the cron job that thought it
	// had asked for 834 requests rather than 83,445. Said only when it was
	// typed, and not behind progressTo: that reader has no terminal.
	var sampleSet bool
	...
	if sampleSet && *sample == 0 {
		fmt.Fprintln(os.Stderr, "-sample 0 means the whole catalog: sweeping every shard")
	}
	...
	// The directory is created only once a sweep is actually going to happen:
	// everything above this line can fail without leaving anything behind.
	if err := os.MkdirAll(*dir, 0o755); err != nil {
		return fmt.Errorf("create %s: %w", *dir, err)
	}

	// Locked before the manifest is read, not merely before the writing
	// starts. Two runs interleaving their writes is the obvious hazard, but
	// the "already have it" answer below is read out of the same set of files
	// a concurrent sweep is republishing -- deciding there is nothing to do
	// from a directory that is halfway through changing is the same bug with a
	// quieter symptom.
	...
	if upToDate && !*force {
		...
		// The manifest of the snapshot that is current, so `catalog sweep` has
		// exactly one machine-readable answer whether or not it had work to
		// do. Exiting 0 with an empty stdout was indistinguishable from a run
		// that produced no output for some other reason; completedAt is what
		// tells "swept just now" from "already had it".
		return emitOne(prev)
	}
	if havePrev && prev.Generation.ID() == gen.ID() && !prev.complete() {
		// What this run is about to do, not what a full run would do: saying
		// "in full" while sampling described a sweep that was not happening.
		what := "in full"
		if *sample > 0 {
			what = fmt.Sprintf("at %g%%", *sample)
		}
		fmt.Fprintf(os.Stderr, "the snapshot for %s covers %g%% of the catalog; sweeping it %s\n",
			gen.ID(), prev.SamplePct, what)
	}
	...
	return sweep(ctx, client, *dir, gen, shards, prev, havePrev, c.concurrency, samp, *keep)
}
...
func sweep(
	...
	// A write failure is terminal: bufio errors are sticky, so every shard
	// fetched after the first one is work that cannot be kept. Cancelling
	// stops dispatch instead of discovering it at the final flush, 80,000
	// requests on.
	sweepCtx, cancelSweep := context.WithCancel(ctx)
	defer cancelSweep()

	var writeErr error
	// failWrite is called with mu held, from inside the shard callback.
	failWrite := func(what string, err error) {
		if writeErr == nil {
			writeErr = fmt.Errorf("%s: %w", what, err)
			cancelSweep()
		}
	}

	// Resume state for a generation that has rolled is unusable: it names shard
	// URLs that no longer exist, and nothing will ever read it again. It was
	// also never deleted -- only the current generation's files were -- so an
	// interrupted sweep left about 99MB behind every time the catalog was
	// republished, which is on the order of 9GB a year.
	if freed := removeStaleResumeState(dir, gen); freed > 0 {
		fmt.Fprintf(os.Stderr, "removed %.0f MB of resume state from generations that have rolled\n",
			float64(freed)/1e6)
	}
	...
	if err != nil {
		// partial is already open, and returning past it leaks the handle.
		_ = partial.Close()
	...
	// loadState returns the previous run's Failed list, so a resumed sweep
	// appended URLs it already held: the retry pass fetched them twice, done
	// was incremented twice, and remaining = len(pending) - done went
	// negative. The set is kept beside the slice rather than replacing it,
	// because the state file's order is what the retry pass walks.
	failedSeen := make(map[string]struct{}, len(state.Failed))
	for _, u := range state.Failed {
		failedSeen[u] = struct{}{}
	}
	// Called with mu held, at both failure sites.
	noteFailed := func(url string) {
		if _, dup := failedSeen[url]; dup {
			return
		}
		failedSeen[url] = struct{}{}
	...
	sweepErr := parallelShards(sweepCtx, pending, concurrency, func(ctx context.Context, url string) {
		...
		for _, pkg := range pkgs {
			if _, werr := fmt.Fprintln(w, pkg); werr != nil {
				failWrite("write "+partialPath, werr)
				return
			}
		}
		state.IDs += len(pkgs)
		if _, werr := fmt.Fprintln(dw, url); werr != nil {
			failWrite("write "+donePath, werr)
			return
		}
	...
	// Before the checkpoint below, because parallelShards returns
	// sweepCtx.Err(): falling through would report a full disk as
	// context.Canceled, which main turns into "interrupted" and exit 130.
	if writeErr != nil {
		_ = checkpoint() // best effort; the sticky writer will report its own error
		_ = partial.Close()
		_ = doneLog.Close()
		return fmt.Errorf("%w; the sweep stopped rather than fetching the rest "+
			"(progress is kept in %s)", writeErr, donePath)
	}
	...
	if len(state.Failed) > 0 && sweepCtx.Err() == nil && !rolled {
		retryURLs := state.Failed
		// Both, together: the retry pass rebuilds the list from what fails
		// again, and a set that still held the old URLs would swallow every
		// one of those.
		state.Failed, failedSeen = nil, map[string]struct{}{}
		...
		retryErr := parallelShards(sweepCtx, retryURLs, concurrency, func(ctx context.Context, url string) {
	...
	return finish(dir, gen, partialPath, state, prev, havePrev, started, samp, keep)
}
...
func finish(
	...
	// Whether there is a delta to write is exactly the question `catalog diff`
	// already answers, and for a while this asked a weaker version of it: the
	// generation id alone. A -sample run after a complete one then wrote
	// "removed" for every id outside the sample -- 3,260 of 5,000 in the
	// repro, about 3.5M in production -- while `catalog diff` refused the same
	// pair outright.
	switch {
	case !havePrev || prev.Generation.ID() == gen.ID():
		// Nothing to diff against, or a -force rerun of the generation already
		// on disk: an empty delta claiming a week of no change that never
		// happened.
	case prev.SamplePct != samp.Pct:
		// The difference between a full snapshot and a sample of the next one
		// is the sampling, not the catalog.
		fmt.Fprintf(os.Stderr, "no delta: %s covered %s and this sweep covered %s; "+
			"the difference would be the sampling, not the catalog\n",
			prev.Generation.ID(), coverageLabel(prev.SamplePct), coverageLabel(samp.Pct))
	case samp.Pct > 0:
		// Equal coverage is not enough once either side is a sample: the seed
		// is derived from the generation id, so two samples of two generations
		// cover almost disjoint sets of apps by construction.
		fmt.Fprintf(os.Stderr, "no delta: each generation samples its own shards, so %s and %s "+
			"cover different apps; the difference would be the sampling, not the catalog\n",
			prev.Generation.ID(), gen.ID())
	default:
		...
		if err := writeJSON(filepath.Join(dir, deltaName(prev.Generation, gen)), d); err != nil {
			return err
		}
	...
	// Snapshots are the caller's data, so they go only when asked. A snapshot
	// is about 43MB and Google republishes every few days, which is on the
	// order of 4GB a year that nothing would otherwise remove -- but deleting
	// history as a side effect of collecting more of it is not a default.
	if keep > 0 {
		if freed, n := pruneSnapshots(dir, keep); n > 0 {
			fmt.Fprintf(os.Stderr, "kept the %d newest snapshots, removed %d older (%.0f MB)\n",
				keep, n, float64(freed)/1e6)
		}
	}
	...
	// The manifest, on stdout, as one NDJSON record. The flagship command was
	// the only one with no machine-readable answer: a consumer had to parse
	// stderr or go reading files out of a directory whose layout this tool
	// calls its own business, which meant depending on exactly the thing it
	// asks nobody to depend on.
	return emitOne(m)
}
...
// writeSnapshotFile writes through a temporary file, fsyncs it, renames it
// over the target and fsyncs the directory -- the durability writeJSON already
// has, and for a stronger reason: the manifest is written afterwards and
// carries this file's sha256, so a snapshot that is present but not durable
// makes the manifest a lie about hours of work.
//
// The hash is taken over the bytes that reach the file, which is what the
// manifest's sha256 has always meant: for the serial path the gzip writer has
// to stay inside the callback for that to hold.
func writeSnapshotFile(path string, write func(io.Writer) error) (string, error) {
	tmp := path + ".tmp"
	f, err := os.Create(tmp)
	if err != nil {
		return "", fmt.Errorf("create %s: %w", tmp, err)
	...
	fail := func(what string, err error) (string, error) {
		_ = f.Close()
		_ = os.Remove(tmp)
		return "", fmt.Errorf("%s %s: %w", what, tmp, err)
	}
...
// maxLineBytes is the longest line any reader here accepts. It was already the
// ceiling every bufio.Scanner in this file is given; readLines, which reads
// the file whole and splits it, was the one reader with no limit at all.
//
// A package id is a few hundred bytes at the outside, so a line past a
// megabyte is not an id: it is a partial file something else wrote into, or
// one whose newlines were lost. Read silently, that becomes a snapshot entry a
// megabyte long and a manifest that counts it as an app.
...
func readLines(path string) ([]string, error) {
	...
	for i, l := range lines {
		if len(l) > maxLineBytes {
			return nil, fmt.Errorf("%s: line %d is %d bytes, over the %d-byte limit; "+
				"a file of app ids has no line that long", path, i+1, len(l), maxLineBytes)
		}
...
// radixMaxDepth bounds the recursion, which is otherwise one stack frame per
// byte of shared prefix.
//
// Package ids share three or four leading bytes, so on the input this is for
// the cap is never reached and costs nothing. It is there for the input this
// is not for: partial-<gen>.txt is a file on disk, and a truncated write, a
// filesystem that returned zeroes, or a path someone pointed at the wrong file
// can hand this a million strings with a megabyte of prefix in common.
// Sixty-four bytes is far past anything an id does and far short of a stack
// that matters, and slices.Sort finishes the job from any depth -- it
// compares whole strings, so the cap changes how the sort gets there and not
// what it produces.
const radixMaxDepth = 64

func radixSort(a []string, depth int) {
	if len(a) <= radixCutoff || depth >= radixMaxDepth {
		slices.Sort(a)
		return
	}
...
// removeStaleResumeState deletes partial and done files belonging to any
// generation but the current one, and returns how many bytes it freed.
//
// These are safe to delete without asking, which is not true of anything else
// in this directory: a done log is a list of shard URLs, and once Google
// republishes, every one of those URLs is gone. The file cannot be resumed
// from, compared against, or read for ids that are not already in the snapshot
// beside it. Nothing had ever deleted them -- only the current generation's
// files were cleaned -- so an interrupted sweep left about 99MB behind every
// time the catalog was republished.
func removeStaleResumeState(dir string, keep googleplayscraper.Generation) int64 {
	var freed int64
	for _, spec := range []struct{ prefix, suffix string }{
		{"partial-", ".txt"},
		{"done-", ".log"},
	} {
		matches, err := filepath.Glob(filepath.Join(dir, spec.prefix+"*"+spec.suffix))
		if err != nil {
			continue
		}
		for _, path := range matches {
			id := strings.TrimSuffix(strings.TrimPrefix(filepath.Base(path), spec.prefix), spec.suffix)
			if id == keep.ID() {
				continue
			}
			// Only files this tool named: <date>_<run>. Anything else in the
			// directory belongs to someone else.
			if date, run, ok := strings.Cut(id, "_"); !ok || date == "" || run == "" {
				continue
			}
...
// pruneSnapshots keeps the newest keep generations in dir and deletes the
// snapshot, manifest and delta of every older one. It returns the bytes freed
// and how many generations went.
//
// Generations are ordered by their run id, which is a build timestamp, so
// "newest" is well defined without reading any file's mtime -- a copied
// directory keeps its meaning.
func pruneSnapshots(dir string, keep int) (int64, int) {
	// keep <= 0 means keep everything, which is the flag's default. Without
	// this the slice below is gens[0:] and the function deletes the lot -- a
	// caller today happens to guard it, which is not the same as being safe.
	if keep <= 0 {
		return 0, 0
	}
	...
	var gens []googleplayscraper.Generation
	for _, path := range manifests {
		id := strings.TrimSuffix(strings.TrimPrefix(filepath.Base(path), "manifest-"), ".json")
		date, run, ok := strings.Cut(id, "_")
		if !ok || date == "" || run == "" {
			continue
		}
		gens = append(gens, googleplayscraper.Generation{Date: date, Run: run})
	}
	slices.SortFunc(gens, func(a, b googleplayscraper.Generation) int { return b.Compare(a) })
	if len(gens) <= keep {
		return 0, 0
	}
	...
	for _, g := range gens[keep:] {
		var any bool
		paths := []string{
			filepath.Join(dir, "snapshot-"+g.ID()+".txt.gz"),
			filepath.Join(dir, "manifest-"+g.ID()+".json"),
		}
		// A delta names two generations, so it is found by either. Once one
		// end of it is gone the file describes a transition into or out of a
		// snapshot that is no longer here. A generation id is <date>_<run> and
		// carries no glob metacharacters.
		if deltas, gerr := filepath.Glob(filepath.Join(dir, "delta-*"+g.ID()+"*.json")); gerr == nil {
			paths = append(paths, deltas...)
		}
